...
for market in markets:
    ...
    option_dataframe["floor"] = option_dataframe["floor_strike"]
    option_dataframe["cap"] = option_dataframe["cap_strike"]
...
account_balance = balanceResponse.balance / 100
trading_amount = 10

orderUuid = str(uuid.uuid4())

trading_contract = pd.json_normalize(kalshi_api.get_market(ticker=chosen_option["ticker"].iloc[0]).to_dict()['market'])
contract_ticker = trading_contract['ticker'].iloc[0]
contract_price = trading_contract['yes_ask'].iloc[0]
contracts_to_trade = round(trading_amount / ((contract_price)/100))

orderResponse = kalshi_api.create_order(CreateOrderRequest(ticker=contract_ticker,action='buy',type="limit", yes_price=contract_price+5, count=contracts_to_trade, client_order_id=orderUuid, side='yes'))
